...
# ---- Fixed GA Parameters (now match Question 2) ----
POP_SIZE = 300          # 1) population setup is 300
CHROM_LEN = 80          # 3) length of individuals is 80
TARGET_ONES = 50        # 2) optimum at ones == 50
MAX_FITNESS = 80        # 4) return value is 80 when ones == 50
N_GENERATIONS = 50      # 5) number of generations is 50
...
def fitness(individual: np.ndarray) -> float:
    ...
    f(ones) = MAX_FITNESS - |ones - TARGET_ONES|
    --> maximum value 80 when ones = 50.
    """
...
if run_btn:
    # Reproducibility (optional but nice for demos)
    random.seed(seed)
    np.random.seed(seed)
